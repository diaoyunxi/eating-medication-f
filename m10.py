...
# ============== 配置区 ==============
BASE_URL = os.environ.get("BASE_URL", "https://my-website.ccwu.cc/eating-medication/family")
...
def set_system_volume(vol):
    ...
    try:
        subprocess.run(["amixer"] + _volume_control_cmd.split() + [f"{vol}%"], timeout=5)
    except Exception as e:
...
def capture_photo(filename=None):
    ...
    try:
        ...
        r = subprocess.run(["fswebcam", "-r", "640x480", "--no-banner", path], capture_output=True, timeout=10)
